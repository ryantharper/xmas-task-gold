...
class Store:
    ...
    def createTableCurrentOrders(self):
        # Deletes if table already exists (useful for when logging into another acc. or when basket is bought)
    ...
    def insert_data_current_order(self, itemId, qty):
        self.c.execute("SELECT saleCost FROM items WHERE id=?", (itemId,))
    ...
    def getItemDetails_CurrentOrder(self):
        ...
        for n in currentOrders:
            self.c.execute("SELECT name FROM items WHERE id=?", (n[1],))
            n.append(n[1]) # used for when buying -- adds item ID to list
            n[1] = self.c.fetchall()[0][0] # sets ID equal to name

        return currentOrders

    def removeOrder(self, id_):
        self.conn.execute("DELETE FROM current_order WHERE id=?", (id_,))
        self.conn.commit()
    ...
    def insert_data_order(self, custId, itemId, qty):
        # Gets bought item sale cost [what the customer bought the item for]
        self.c.execute("SELECT saleCost FROM items WHERE id=?", (itemId,))
        costTotal = self.c.fetchone()[0]
        # Gets bought item purchase cost [what the store bought the item for]
        self.c.execute("SELECT purchCost FROM items WHERE id=?", (itemId,))
        purchCost = self.c.fetchone()[0]
        # Calculates total cost
        cost = int(qty) * costTotal
        # Calculates how much profit earned from order
        profit = cost-(int(qty)*purchCost)
        # Inserts order details into orders table & updates customers orders
        ...
        self.conn.commit()
        # Runs top5customers function so if user is now top 5 it updates the customers table and top5 list.
        self.top5Customers()

    # --------------------- others ---------------------

    def deleteUser(self, custId):
        self.conn.execute("DELETE FROM customers WHERE id=?", (custId,))
        self.conn.execute("DELETE FROM orders WHERE custId=?", (custId,))
        self.conn.commit()
        # Updates top5customers 
        self.top5Customers()
    ...
    def updateStock(self, idn, change):
        self.c.execute("SELECT stock_lvl FROM items WHERE id=?", (idn,)) # gets stock_lvl of the item corresponding to id
        currentStock = self.c.fetchall()[0][0] # gets stock_lvl
        newStock = currentStock + int(change) # alters stock level
        self.conn.execute("UPDATE items SET stock_lvl = ? WHERE id=?", (newStock, idn,)) # updates stock level to new stock level
        self.conn.commit() # commits changes to table

    # SHOW ORDER HISTORY FOR USER

    def showOrderHistory(self,custId):
        self.c.execute("SELECT * FROM orders WHERE custId=?", (custId,))
        ...
        for i in orders:
            self.c.execute("SELECT name FROM items WHERE id=?", (i[2],))
    ...
    def printItems(self, usr):
        # For shoppers (doesn't show item where stock level = 0 )
        if usr == 1: 
            self.c.execute("SELECT id, name, round(purchCost, 2), round(saleCost, 2), stock_lvl, cat, quantity_bought FROM items WHERE stock_lvl > 0")
            return self.c.fetchall()
        # For workers
        else:
            self.c.execute("SELECT id, name, round(purchCost, 2), round(saleCost, 2), stock_lvl, cat, quantity_bought FROM items") 
            return self.c.fetchall()

    def updateUserAcc(self, custId, col, new):
        self.conn.execute("UPDATE customers SET ? = '?' WHERE id=?", (col, new, custId,))
        self.conn.commit()
    ...
    def top5products(self):
        ...
        # Sorts item list by quantity bought, then splices so it is only top 5
        itemListT5 = sorted(itemList, key=lambda t: t[1], reverse=True)[:5]
    ...
    def top5custs(self):
        ...
        for i in custIdList:
            self.c.execute("SELECT profit FROM orders WHERE custId=?", (i[0],))
            profits = self.c.fetchall()

            self.c.execute("SELECT cost FROM orders WHERE custId=?", (i[0],))
        ...
        # Sorts customerList by profit made, then splices so it is only top 5
        # reverse = True sorts it from z-a
        # key is what the list is sorted by
        ...
        for i in custListT5:
            self.conn.execute("UPDATE customers SET top5 = 1 WHERE id=?", (i[0],))
            self.conn.commit()
