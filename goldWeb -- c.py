...
# Shopper Page - Login, Create Account, Back to Index
@route('/shopper')
def mainShopper():
...
def loginPagePost():
    ...
    if username in validUsernames:
        store.c.execute("SELECT id FROM customers WHERE username='?'", (username,))
        shopperid = store.c.fetchone()[0] # (was sid=..)
        # Creates table for Basket [current orders]
        store.createTableCurrentOrders()
        # Sets session variable, sid, equal to the users id
        s['sid'] = shopperid
...
def shopperPage2():
    ...
    for k in request.forms:
        ...
        # BUY BASKET
            ...
            # buys basket, updating stock + inserting orders for basket 
            for order in basket:
                ...
                store.createTableCurrentOrders()
                # +1 in orders:
                store.conn.execute("UPDATE customers SET orders = orders + ? WHERE id=?", (order[2], userId,))
                store.conn.execute("UPDATE items SET quantity_bought = quantity_bought + ? WHERE id=?", (order[2], order[4],))
                store.conn.commit()
...
def editAccountPage():
    # If there is an error (from URL) -- show it on page
    try:
        pageError = request.query.error
    except:
        pageError = "Hello, Shopper!"

    # creates session & gets session variable [the user ids]
    s = request.environ.get('beaker.session')
    userId = s['sid']
    
    store.c.execute("SELECT * FROM customers WHERE ID=?", (userId,))
    userDetails = store.c.fetchall()

    return template('editaccount', details=userDetails, error=pageError)

# EDIT ACCOUNT - POST, checks if username exists, if it does, reload the page with the error.
